...
本文件用于记录“迹目”的产品决策和开发路线。T1-T7 已完成后，本文件从功能建设路线切换为发布准备路线；更详细的发行分析参考 `RELEASE_PLAN.md`。
...
当前阶段判断：T1-T7 已经收口，MVP 功能闭环成立。项目可以从“功能建设阶段”切换到“发布准备阶段”，但当前还不是可直接上架的正式发行版：release 产物仍是 unsigned APK，缺少正式签名配置、隐私政策、品牌图标、数据导出/导入和若干体验打磨。
...
- 复盘历史列表：数据层已有 `observeAllReviews()`，UI 缺口小，能补齐复盘“回看”价值。
...
6. R11-R14：补设置页、隐私政策和复盘历史列表。
7. R15-R17：补核心测试和公共 UI 组件整理。

这样推进的好处是：T1-T7 已经证明核心产品闭环可用；下一阶段优先解决“能否放心交给别人安装和长期使用”的问题，而不是继续扩张新功能。
