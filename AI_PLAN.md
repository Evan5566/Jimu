...
本文件用于记录“迹目”的产品决策和开发路线。T1-T8 已完成后，本文件从功能建设路线切换为发布准备路线；更详细的发行分析参考 `RELEASE_PLAN.md`。
...
当前阶段判断：T1-T8 已经收口，MVP 功能闭环成立，且复盘“回看”能力已补齐。项目可以从“功能建设阶段”切换到“发布准备阶段”，但当前还不是可直接上架的正式发行版：release 产物仍是 unsigned APK，缺少正式签名配置、隐私政策、品牌图标、数据导出/导入和若干体验打磨。
...
当前不建议推倒重做 UI。现有 Material3 + Compose 结构、蓝色主题、页面分组和动画基础可以继续沿用；发布前重点做图标、深色模式、Typography、删除确认、设置页和数据安全。
...
1. R0：确认并提交发布路线文档。`RELEASE_PLAN.md` 可作为详细分析保留，`AI_PLAN.md` 作为主路线。
2. R1：先确定 App 图标方案并替换默认图标。
3. R2：配置 release 签名与版本策略，生成可安装的 signed release 包。
4. R3-R7：补齐深色模式、Typography、启动画面、待办/习惯删除确认、`INTERNET` 权限清理。
5. R8-R10：做数据导出/导入和数据层安全债整改。
6. R12-R14：补设置页、隐私政策；R11 复盘历史列表已在 T8 完成。
7. R15-R17：补核心测试和公共 UI 组件整理。

这样推进的好处是：T1-T8 已经证明核心产品闭环和回看能力可用；下一阶段优先解决“能否放心交给别人安装和长期使用”的问题，而不是继续扩张新功能。
