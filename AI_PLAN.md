...
本文件用于记录“迹目”的产品决策和开发路线。T1-T8 已完成后，路线先进入 T9 本地复盘草稿 MVP，用于验证“复盘减负”和核心留存价值；T9 验证后再进入发布准备路线。更详细的发行分析参考 `RELEASE_PLAN.md`。
...
- 复盘闭环已具备：首页“今日复盘”卡片、今日复盘编辑页、保存后首页显示摘要、再次进入预填修改、历史复盘列表和指定日期编辑。

## 近期路线

当前阶段判断：T1-T8 已经收口，MVP 功能闭环成立，且复盘“回看”能力已补齐。但当前产品主线仍需要一次小闭环验证：用现有待办、习惯、目标数据生成本地复盘草稿，降低用户每天手填复盘的成本。T9 应优先于 R1-R7 发布准备任务；T9 验证成立后，再决定是否把底部“已完成”降级为复盘页子模块，并进入正式发布准备。
...
- T9：计划执行 - 本地复盘草稿 MVP。
  - 新增 `DailyDigestRepository` 或 `ReviewDraftRepository` 作为业务聚合层。
  - 聚合今日待办、习惯打卡、目标步骤推进、未完成/逾期提醒。
  - 在现有 `ReviewScreen` 顶部展示“今日成果草稿”，下面保留三段手填复盘。
  - 今日完成待办先采用低风险近似口径，不承诺精确完成时间。
  - 不改 `ReviewEntity`，不升级 Room version，不写 migration，不新增 `completedAt`。
  - 不引入云服务、大模型 API、复杂统计图、复盘删除或复杂模板。
  - 不立即把底部“已完成”替换成“复盘”，避免在价值验证前做导航大改。
...
发布准备不再直接从 R1 开始。当前主线调整为：T9 复盘草稿 MVP -> 评估底部导航重排 -> R1-R7 发行基础 -> R8-R10 数据安全 -> R12-R17 体验与质量补齐。
...
1. T9：本地复盘草稿 MVP，先验证复盘减负和今日成果反馈。
2. T10：根据 T9 结果评估底部导航重排，决定是否把“已完成”降级为复盘页子模块。
3. R1-R7：补齐 App 图标、release 签名、深色模式、Typography、启动画面、删除确认和 `INTERNET` 权限清理。
4. R8-R10：做数据导出/导入和数据层安全债整改。
5. R12-R14：补设置页、隐私政策；R11 复盘历史列表已在 T8 完成。
6. R15-R17：补核心测试和公共 UI 组件整理。
